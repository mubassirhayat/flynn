...
func (s *service) Leaders(leaders chan *Instance) (stream.Stream, error) {
	res, err := s.client.c.RawReq("GET", fmt.Sprintf("/services/%s/leader", s.name), http.Header{"Accept": {"text/event-stream"}}, nil, nil)
	if err != nil {
		return nil, err
	}
	stream := stream.New()
	go func() {
		events := make(chan *Event)
		eventStream := httpclient.Stream(res, events)
		defer func() {
			eventStream.Close()
...
func (s *service) Watch(events chan *Event) (stream.Stream, error) {
	res, err := s.client.c.RawReq("GET", fmt.Sprintf("/services/%s", s.name), http.Header{"Accept": {"text/event-stream"}}, nil, nil)
	if err != nil {
		return nil, err
	}
	return httpclient.Stream(res, events), nil
}
